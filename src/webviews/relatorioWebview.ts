...
import { carregarOrcamento } from '../services/orcamentoService';
import {
    gerarRelatorioDiarioComDados,
    gerarRelatorioFaseComDados,
    gerarRelatorioCompletoComDados
} from '../services/relatorioService';
import { escapeAttr, escapeHtml } from '../utils/format';

type TipoRelatorio = 'diario' | 'fase' | 'completo';

export function abrirRelatorioWebview(
    ...
    panel.webview.onDidReceiveMessage(async message => {
        ...
        if (tipo === 'diario') {
            await gerarRelatorioDiarioComDados(paths, message.data);
        }

        if (tipo === 'fase') {
            await gerarRelatorioFaseComDados(paths, message.data);
        }

        if (tipo === 'completo') {
            await gerarRelatorioCompletoComDados(paths, message.data);
        }
...
function getHtml(tipo: TipoRelatorio, config: any): string {
    const fasesOptions = config.fases.map((fase: any) => {
        return `<option value="${escapeAttr(fase.id)}">${escapeHtml(fase.nome)}</option>`;
    }).join('');
...
<div class="card">
    ...
    <div class="row">
        <div>
            <label>Cliente</label>
            <input id="clienteNome" value="${escapeAttr(config.cliente.nome)}">
        </div>
        <div>
            <label>Prestador</label>
            <input id="prestadorNome" value="${escapeAttr(config.prestador.nome)}">
        </div>
    </div>
</div>
...
<div class="card">
    ...
    <textarea id="resumoProjeto">${escapeHtml(config.descricaoProjeto || '')}</textarea>
