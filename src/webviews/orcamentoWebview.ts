...
import { FreelaPaths, OrcamentoConfig } from '../types/freela';
import { carregarOrcamento, salvarOrcamento } from '../services/orcamentoService';
import { escapeHtml, escapeAttr } from '../utils/format';

export function abrirOrcamentoWebview(
    context: vscode.ExtensionContext,
    paths: FreelaPaths
): void {
    const dados = carregarOrcamento(paths);
    ...
    panel.webview.onDidReceiveMessage(async message => {
        if (message.command !== 'salvar') return;

        await salvarOrcamento(paths, message.data as OrcamentoConfig);

        vscode.window.showInformationMessage(
            'Orçamento salvo em .freela/config.json, .md e .html.'
        );
...
function getHtml(d: OrcamentoConfig): string {
    return `<!DOCTYPE html>
<html lang="pt-BR">
<head>
<meta charset="UTF-8">
...
<div class="row">
    <div><label>Nome</label><input id="prestadorNome" value="${escapeAttr(d.prestador.nome)}"></div>
    <div><label>CPF/CNPJ</label><input id="prestadorDocumento" value="${escapeAttr(d.prestador.documento)}"></div>
</div>
<div class="row">
    <div><label>E-mail</label><input id="prestadorEmail" value="${escapeAttr(d.prestador.email)}"></div>
    <div><label>Telefone</label><input id="prestadorTelefone" value="${escapeAttr(d.prestador.telefone)}"></div>
</div>

<h2>2. Dados do Cliente</h2>
<div class="row">
    <div><label>Nome</label><input id="clienteNome" value="${escapeAttr(d.cliente.nome)}"></div>
    <div><label>CPF/CNPJ</label><input id="clienteDocumento" value="${escapeAttr(d.cliente.documento)}"></div>
</div>
<div class="row">
    <div><label>E-mail</label><input id="clienteEmail" value="${escapeAttr(d.cliente.email)}"></div>
    <div><label>Telefone</label><input id="clienteTelefone" value="${escapeAttr(d.cliente.telefone)}"></div>
</div>

<h2>3. Projeto</h2>
<label>Nome do projeto</label>
<input id="projetoNome" value="${escapeAttr(d.projetoNome)}">

<label>Descrição do projeto</label>
<textarea id="descricaoProjeto">${escapeHtml(d.descricaoProjeto)}</textarea>

<label>Escopo do projeto</label>
<textarea id="escopoProjeto">${escapeHtml(d.escopoProjeto)}</textarea>

<label>Premissas / itens que o cliente vai disponibilizar</label>
<textarea id="premissasCliente">${escapeHtml(d.premissasCliente)}</textarea>

<h2>4. Metodologia de Trabalho</h2>
<textarea id="metodologiaTrabalho">${escapeHtml(d.metodologiaTrabalho)}</textarea>
...
<textarea id="revisaoProjeto">${escapeHtml(d.revisaoProjeto)}</textarea>

<label>Suporte após entrega</label>
<textarea id="suporteAposEntrega">${escapeHtml(d.suporteAposEntrega)}</textarea>

<label>Garantias e observações</label>
<textarea id="garantiasObservacoes">${escapeHtml(d.garantiasObservacoes)}</textarea>

<h2>7. Financeiro</h2>
<label>Valor/hora padrão</label>
<input id="valorHoraPadrao" value="${escapeAttr(d.valorHoraPadrao)}">
...
<div class="row">
    ...
    <div>
        ...
        <input id="descontoValor" value="${escapeAttr(d.condicoesFinanceiras.descontoValor)}" placeholder="Ex: 10% ou 300,00">
    </div>
</div>

<label>Parcelamento</label>
<textarea id="parcelamento">${escapeHtml(d.condicoesFinanceiras.parcelamento)}</textarea>

<label>Regra personalizada de pagamento</label>
<textarea id="regraPagamentoPersonalizada">${escapeHtml(d.condicoesFinanceiras.regraPagamentoPersonalizada)}</textarea>

<label>Regra sobre envio do código-fonte</label>
<input id="percentualCodigoFonte" value="${escapeAttr(d.condicoesFinanceiras.percentualCodigoFonte)}" placeholder="Ex: código-fonte enviado após 100% do pagamento">

<label>Observações financeiras</label>
<textarea id="observacoesFinanceiras">${escapeHtml(d.condicoesFinanceiras.observacoesFinanceiras)}</textarea>
